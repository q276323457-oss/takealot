...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("OKX RIVER 趋势合约 中文按钮界面 v2")
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("OKX RIVER 趋势合约 中文按钮界面 v2")
        title.setStyleSheet("font-size: 20px; font-weight: 700;")
        layout.addWidget(title)

        subtitle = QLabel("当前默认：v2 保守趋势版。5m 执行 + 15m/30m 趋势过滤。高频 v3 已验证为过度交易，不建议默认使用。")
        ...
        status_layout.addWidget(QLabel("合约机器人"), 0, 4)
        ...
        config_layout.addWidget(QLabel("OKX 口令"), 3, 0)
        self.api_password = QLineEdit()
        self.api_password.setEchoMode(QLineEdit.EchoMode.Password)
        self.api_password.setPlaceholderText("OKX 必填：这里填 API Passphrase")
        ...
        config_hint = QLabel("提示：只做回测可以先不填 API。要启动 OKX 合约机器人，Key / Secret / Passphrase 要填全。")
        config_hint.setStyleSheet("color: #666;")
        config_layout.addWidget(config_hint, 4, 0, 1, 6)
        ...
        self.download_range = QLineEdit("20260109-")
        self.download_range.setPlaceholderText("例如 20260109- 或 20260109-20260322")
        ...
        self.backtest_range = QLineEdit("20260110-")
        ...
        btn_dry_run = QPushButton("4 启动合约模拟盘")
        btn_dry_run.clicked.connect(lambda: self._run_command("启动合约模拟盘", "bash scripts/03_dry_run.sh"))
        ...
        btn_stop_bot = QPushButton("停止合约模拟盘")
        btn_stop_bot.clicked.connect(lambda: self._run_command("停止合约模拟盘", "bash scripts/04_stop.sh"))
    ...
    def _download_data(self) -> None:
        timerange = self.download_range.text().strip() or "20260109-"
    ...
    def _backtest(self) -> None:
        timerange = self.backtest_range.text().strip() or "20260110-"
    ...
    def _humanize_backtest(self, summary: dict) -> tuple[str, str, str, str]:
        ...
        if trades < 6:
            return (
                "样本太少",
                "#f4a261",
                "RIVER 合约历史还短，这次交易次数太少，先别急着下结论。",
                "先继续补数据、反复回测，至少让样本更多一点。",
            )
        ...
        if profit_pct <= 0:
            return (
                "不建议用",
                "#e76f51",
                "这次 RIVER 趋势回测总体亏钱，先不要上真钱。",
                "先继续改趋势规则或风控，不要急着实盘。",
            )

        if profit_pct < 3 or profit_factor < 1.15:
            return (
                "能跑但偏弱",
                "#e9c46a",
                "这套 RIVER 趋势机器人能跑，但优势还不够明显。",
                "可以继续观察合约模拟盘，也可以让我继续优化 v2。",
            )
        ...
        if drawdown_pct > 8:
            return (
                "收益还行但风险偏高",
                "#f4a261",
                "看起来能赚，但 RIVER 波动太大，回撤还是偏高。",
                "先把止损和杠杆再收紧，再考虑长时间跑模拟盘。",
            )

        return (
            "结果不错",
            "#2a9d8f",
            "这次 RIVER 趋势回测表现还可以，可以继续跑合约模拟盘验证。",
            "先别直接上真钱，至少先跑 14 到 30 天模拟盘。",
        )
